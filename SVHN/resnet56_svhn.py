...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10):
        ...
        # ResNet56 has layers: [9, 9, 9] for 3 stages
...
# SVHN Dataset with proper handling
def load_svhn_data(data_dir='./svhn_data'):
    """
    Load SVHN dataset
    Args:
        data_dir: Directory to save/load SVHN data
    Returns:
        train_dataset, test_dataset, extra_dataset
    """
    print(f"Loading SVHN data from/to {data_dir}")
    
    # Create directory if it doesn't exist
    os.makedirs(data_dir, exist_ok=True)
    
    # Define transformations
    # For SVHN, we don't normalize with ImageNet stats since it's not ImageNet data
    # Using stats appropriate for SVHN
    ...
    try:
        # Load train set (73,257 images)
        train_dataset = SVHN(root=data_dir, split='train', download=True, transform=train_transform)
        
        # Load test set (26,032 images)
        test_dataset = SVHN(root=data_dir, split='test', download=True, transform=test_transform)
        
        # Load extra set (531,131 additional images, optional)
        try:
            extra_dataset = SVHN(root=data_dir, split='extra', download=True, transform=train_transform)
            print(f"Number of extra samples: {len(extra_dataset)}")
        except:
            print("Extra dataset not available or failed to load. Using only train and test sets.")
            extra_dataset = None
        ...
        print("\nDataset Statistics:")
        print(f"Train images shape: {train_dataset.data.shape}")
        print(f"Test images shape: {test_dataset.data.shape}")
        
        # Show label distribution for train set
        ...
        print(f"Error loading SVHN dataset: {e}")
...
def run_experiment(seed_idx, seed, experiment_config, data_dir='./svhn_data'):
    """
    ...
    # Load data
    train_set, test_set, extra_set = load_svhn_data(data_dir)
    
    # Use only the standard train set (73,257 images) for experiments
    # Optionally, you could incorporate the extra set here
...
def run_all_experiments(data_dir='./svhn_data'):
...
def run_specific_experiment(exp_number, data_dir='./svhn_data'):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Run ResNet56 experiments on SVHN')
    ...
    parser.add_argument('--data-dir', type=str, default='./svhn_data',
                       help='Path to SVHN dataset directory')
    parser.add_argument('--use-extra', action='store_true',
                       help='Use SVHN extra dataset (531K additional images)')
