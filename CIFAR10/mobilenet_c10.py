...
# Define MobileNetV2 model for CIFAR-10
...
def MobileNetV2_CIFAR10():
    return MobileNetV2(num_classes=10).to(device)
...
def load_best_model_from_folder(folder_path):
    ...
    if not model_files:
        ...
        try:
            ...
            best_model = MobileNetV2_CIFAR10().to(device)
...
def run_experiment(seed_idx, seed, experiment_config):
    ...
    # Save to model-dataset.txt
    with open(f"{path}/mobilenetv2-cifar10.txt", "w") as f:
        f.write("\n".join(summary_lines))
...
def run_all_experiments():
    ...
    for exp_idx, experiment in enumerate(experiments):
        ...
        if exp_accuracies and exp_total_times:
            ...
            # Save model-dataset.txt summary
            summary_lines = []
            ...
            with open(f"{exp_dir}/mobilenetv2-cifar10.txt", "w") as f:
                f.write("\n".join(summary_lines))
            
            print(f"\n{'*'*100}")
            print(f"COMPLETED: {experiment['name']}")
            print(f"Model: MobileNetV2, Dataset: CIFAR-10")
    ...
    # Create a master model-dataset.txt file
    master_lines = []
    ...
    master_lines.append(f"Dataset: CIFAR-10")
    master_lines.append("=" * 120)
    ...
    with open("./results/mobilenetv2-cifar10.txt", "w") as f:
        f.write("\n".join(master_lines))
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Run MobileNetV2 experiments on CIFAR-10')
