...
def load_best_model_from_folder(folder_path):
    ...
    if not model_files:
        ...
        try:
            ...
            best_model = VGG('VGG16').to(device)
...
def run_experiment(seed_idx, seed):
    """Run a complete experiment with a given seed"""
    print(f"\n{'='*60}")
    print(f"Running experiment {seed_idx+1} with seed: {seed}")
    print(f"{'='*60}")
    ...
    initial_accuracy, initial_time = train_test_save(initial_trainset, test_set, 1, 200, path, lr=0.01)
    ...
    final_model, final_accuracy, degradation_time = train_until_degradation(model, initial_trainset, remainder, test_set, path)
    
    # Calculate total experiment time
    total_experiment_time = time.time() - experiment_start_time
    
    print(f"\nExperiment {seed_idx+1} (Seed: {seed}) Summary:")
...
def main():
    """Main function to run 5 experiments with different seeds"""
    ...
    print(f"\n{'='*80}")
    print(f"STARTING 5 EXPERIMENTS WITH DIFFERENT SEEDS")
    print(f"{'='*80}")
    
    # Run experiments
    for i, seed in enumerate(seeds):
        accuracy, exp_time = run_experiment(i, seed)
    ...
    if all_accuracies and all_times:
        ...
        print("No valid results obtained from experiments.")
