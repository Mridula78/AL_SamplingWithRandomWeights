...
def train_model(model, train_loader, epochs, learning_rate, accumulate_steps=1):
    ...
    for epoch in range(epochs):
        ...
        if new_accuracy > best_accuracy:
            if os.path.exists(f"/home/user5/Documents/Models/SWIN_LC/model_{best_accuracy:.2f}.pt"):
                os.remove(f"/home/user5/Documents/Models/SWIN_LC/model_{best_accuracy:.2f}.pt")  # Delete the previous model
            best_accuracy = new_accuracy
            # Save the new model with the highest accuracy
            torch.save(model.state_dict(), f"/home/user5/Documents/Models/SWIN_LC/model_{best_accuracy:.2f}.pt")
...
def train_until_degradation(model, train_dataset, remaining_set, degradation_thres=0.03):
    ...
    print("Final accuracy on the validation set:", best_accuracy)
    with open("/home/user5/Documents/Models/SWIN_LC/accuracies.txt", "w") as file:
        for i, acc in enumerate(accuracies, start=1):
            file.write(f"Iteration {i}: Accuracy = {acc:.2f}\n")

    torch.save(model, f"/home/user5/Documents/Models/SWIN_LC/finalmodel_{best_accuracy:.2f}.pt")
    return model
...
model = load_best_model_from_folder("/home/user5/Documents/Models/SWIN_LC")
final_model = train_until_degradation(model,train_set,remainder,degradation_thres=0.3)
