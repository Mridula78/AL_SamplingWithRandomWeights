...
# Define MobileNetV2 model for CIFAR-100
...
def MobileNetV2_CIFAR100():
    return MobileNetV2(num_classes=100).to(device)
...
def load_best_model_from_folder(folder_path):
    ...
    if not model_files:
        ...
        try:
            ...
            best_model = MobileNetV2_CIFAR100().to(device)
...
def run_experiment(seed_idx, seed, experiment_config):
    ...
    # Prepare comprehensive summary for mobilenetv2-cifar100.txt
    summary_lines = []
    summary_lines.append("=" * 80)
    summary_lines.append(f"EXPERIMENT SUMMARY: {experiment_config['name']}")
    summary_lines.append(f"Model: MobileNetV2")
    summary_lines.append(f"Dataset: CIFAR-100")
    ...
    # Save to mobilenetv2-cifar100.txt
    with open(f"{path}/mobilenetv2-cifar100.txt", "w") as f:
        f.write("\n".join(summary_lines))
...
def run_all_experiments():
    ...
    for exp_idx, experiment in enumerate(experiments):
        ...
        if exp_accuracies and exp_total_times:
            ...
            # Save mobilenetv2-cifar100.txt summary
            summary_lines = []
            ...
            with open(f"{exp_dir}/mobilenetv2-cifar100.txt", "w") as f:
                f.write("\n".join(summary_lines))
            
            print(f"\n{'*'*100}")
            print(f"COMPLETED: {experiment['name']}")
            print(f"Model: MobileNetV2, Dataset: CIFAR-100")
    ...
    # Create a master mobilenetv2-cifar100.txt file
    master_lines = []
    master_lines.append("=" * 120)
    master_lines.append("ALL EXPERIMENTS COMPLETE - FINAL COMPARISON")
    master_lines.append(f"Model: MobileNetV2")
    master_lines.append(f"Dataset: CIFAR-100")
    master_lines.append("=" * 120)
    ...
    with open("./model_data/mobilenetv2-cifar100.txt", "w") as f:
        f.write("\n".join(master_lines))
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Run MobileNetV2 experiments on CIFAR-100')
