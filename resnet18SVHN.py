...
train_set = datasets.SVHN(root='./data_svhn', split='train', download=True, transform=transform_train)
test_set = datasets.SVHN(root='./data_svhn', split='test', download=True, transform=transform_test)
...
# Load pretrained ResNet18 from torchvision and copy weights safely
pretrained_model = models.resnet18(weights=models.ResNet18_Weights.DEFAULT)
pretrained_dict = pretrained_model.state_dict()
model_dict = model.state_dict()

# Only copy parameters that match in name and shape (skip conv1, linear)
pretrained_dict = {k: v for k, v in pretrained_dict.items()
                   if k in model_dict and model_dict[k].shape == v.shape and 'linear' not in k}
    ...
    for epoch in range(epochs):
        ...
        if new_accuracy > best_accuracy:
            if os.path.exists(f"{path}/model_{best_accuracy:.2f}.pt"):
                os.remove(f"{path}/model_{best_accuracy:.2f}.pt")
...
def train_until_degradation(model, train_dataset, remaining_set, path):
    ...
    while len(remaining_set) != 0:
        ...
        train_loader = DataLoader(train_dataset, batch_size=64, shuffle=True, num_workers=4)
        current_accuracy = train_model(model, train_loader, 100, 0.01, path)
...
train_test_save(initial_trainset, 1, 100, path, lr=0.01)
